...
class CrossCosmosGui(arcade.Window):
    ...
    def __init__(self, grid: Grid, config: LayoutConfig, df: pl.DataFrame, run_solver: bool = True):
        ...
        self.gps = GridPruningSolver(grid=grid, word_df=df)
    ...
    @property
    def grid(self) -> Grid:
        return self.gps.grid

    @property
    def df(self) -> pl.DataFrame:
        return self.gps.word_df
    ...
    def on_mouse_motion(self, x: int, y: int, dx: int, dy: int):
        ...
        highlight_color = (
            self.cfg.color.block_valid_highlight if temp_grid.is_valid else self.cfg.color.block_invalid_highlight
        )
    ...
    def _setup(self):
        ...
        arcade.load_font(":resources:/fonts/ttf/Liberation/Liberation_Mono_Regular.ttf")
        arcade.load_font(":resources:/fonts/ttf/Liberation/Liberation_Sans_Regular.ttf")
        ...
        # Crossword grid
        xword_grid = self._create_xword_grid(1 - grid_info_ratio)
        left_container.add(xword_grid)

        # Info section below grid
        info_section = self._create_info_section(grid_info_ratio)
        left_container.add(info_section)
        ...
        sidebar_width_ratio = 1 - self.cfg.grid.right_main_ratio
        side_bar = arcade.gui.UIBoxLayout(vertical=True, size_hint=(1 - self.cfg.grid.right_main_ratio, 1))
        side_bar.with_background(color=self.cfg.color.grid_background)
    ...
    def _create_word_list_section(self, **kwargs):
        ...
        right_word_info_uc.add(self.n_letters_label, anchor_x="left", anchor_y="center_y")
        ...
        right_word_info_ul.add(self.current_entry_label, anchor_x="left", anchor_y="center_y")
        ...
        right_word_info_ur.add(self.n_possible_label, anchor_x="left", anchor_y="center_y")
    ...
    def _init_layout_parameters(self):
        ...
        self.left_side_ratio = (self.height - grid_config.bottom_margin + self.cfg.grid.top_margin) / self.width
        # self.left_side_ratio = self.height / self.width
        self.right_side_ratio = 1 - self.left_side_ratio

        # Calculate the widths for each of the sides based on the computed ratios
    ...
    def _create_xword_grid(self, height_pct: float):
        ...
        xword_grid_area = arcade.gui.UIWidget(size_hint=(1, height_pct))
        ...
        for row in range(self.grid.row_count):
            for column in range(self.grid.col_count):
                ...
                self.cell_letters[row, column] = arcade.Text(
                    ...
                    font_name=self.cfg.text.grid_font_name,
                    bold=True,
                )
    ...
    def _create_info_section(self, height_pct: float):
        # ==============================================================
        # Bottom text labels
        # ==============================================================

        # Left - Bottom (Info)
        info_section_area = arcade.gui.UIBoxLayout(vertical=False, size_hint=(1, height_pct))
        ...
        self.grid_location_label = arcade.gui.UILabel(
            size_hint=(1, 0.5), text="(0,0)", font_size=14, font_name=self.cfg.text.grid_font_name
        )
        ...
        self.current_value_label = arcade.gui.UILabel(
            size_hint=(1, 0.5), text='"ASDF??A?BBQPEE??ADFG"', font_size=14, font_name=self.cfg.text.grid_font_name
        )
...
if __name__ == "__main__":
    ...
    if "test_df" not in locals():
        ...
        test_df = Query(default=True, q=1, limit=None).order_by_score().df()
        logger.info(f"Complete! {len(test_df)} loaded.")
    else:
        test_df = None
